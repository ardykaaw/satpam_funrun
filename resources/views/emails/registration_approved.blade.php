...
<html lang="id">
  ...
  <body style="font-family: system-ui, -apple-system, Segoe UI, Roboto, 'Helvetica Neue', Arial, sans-serif; background:#f5f5f5; margin:0; padding:0;">
    <div style="max-width:600px; margin:0 auto; background:#ffffff; padding:0;">
      ...
      <!-- Logo Header - Same as confirmation email -->
      <div style="text-align:center; padding:24px 20px; border-bottom:1px solid #e0e0e0;">
        @php
          // Try multiple paths to find Logo file - same as confirmation email
          $logoPath = null;
          $possiblePaths = [
        ...
        <p style="margin:16px 0 0; color:#333; font-size:15px; line-height:1.6;">
          Email ini merupakan konfirmasi bahwa pendaftaran Anda telah <strong>disetujui</strong> untuk <strong>Satpam Fun Run 5K</strong>
        </p>
      </div>

      <!-- QR Code - Paling Atas -->
      @php
        // Use $message->embed() - EXACT same method as logo in confirmation email (which works)
        // This is the most reliable method for email clients
        $barcodeFilePath = null;
        if ($registration->barcode) {
          $storagePath = str_replace('storage/', '', $registration->barcode);
      ...
      <div style="text-align:center; padding:32px 20px; background:#ffffff;">
        ...
        {{-- Use $message->embed() - EXACT same as logo in confirmation email which works --}}
